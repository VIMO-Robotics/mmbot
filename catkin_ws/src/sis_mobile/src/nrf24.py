...
class Car_controller(object):
	def __init__(self):
		...
		self.port_nrf24 = 'dev/ttyUSB0'
		self.port_motor = '/dev/ttyACM0' # Arduino port from parameter server
		# odom to car_base
		self.arduion_nrf24 = serial.Serial(self.port_nrf24, 9600)
		self.arduino_motor = serial.Serial(self.port_motor, 57600)
		# Flush serial data
		for i in range(0, 20):
			_ = self.arduino_motor.readline()
	...
	def reset_odom(self, req):
		...
		self.y = 0
		self.heading = 0
		self.arduino_motor.write("c".encode()) # Write to the serial to make Arduino clear theta param
	...
	def read_data(self, event):
		...
		if self.shutdown_:
			return
		data_str = self.arduino_motor.readline()
